...
<div class="positionDash" class="wraper">
    <h1 class="text-center positionH" >Перелік видів абонементів   </h1>
    <a href="/viewAdd" class="positionbut c">Створити</a> </span>
      <div>
        ...
        <c:forEach items="${abonementKindList}" var="abKind"  >
             <tr>
                 <td><a href="/changeAbKind/${abKind.id}">${abKind.nameAb}</a></td>
                 <td><a href="/changeAbKind/${abKind.id}">${abKind.daysValid}</a></td>
                 <td><a href="/changeAbKind/${abKind.id}">${abKind.traindirections.name}</a></td>
                 <td><a href="/changeAbKind/${abKind.id}"><fmt:formatDate value="${abKind.dateCreate}" pattern="dd.MM.yyyy"/></a></td>
             </tr>
       </c:forEach>
        </tbody>
    </table>
